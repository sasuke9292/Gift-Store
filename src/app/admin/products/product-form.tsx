...
export default function ProductForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (price <= 0) {
      toast.error('يرجى تحديد سعر صالح للمنتج!')
      setActiveTab('general')
      return
    }
    ...
    try {
      if (product) {
        ...
        const res = await createProduct({
          ...
          salePrice: salePrice > 0 ? salePrice : null,
          categoryId: formData.categoryId,
          images: formData.imagesList.length > 0 ? formData.imagesList : [
        ...
        if (res.success && res.data) {
          toast.success('تمت إضافة المنتج الجديد للمتجر بنجاح 🎉')
          onSuccess(res.data)
        } else {
  ...
  return (
    <form 
      ...
      className={cn(
        "flex flex-col w-full text-start",
        isInline ? "bg-white border border-[#E8E4DF] rounded-3xl shadow-sm overflow-hidden" : "max-h-[90vh]"
      )}
      dir="rtl"
    >
      {/* Header */}
      <div className="p-5 sm:p-6 md:p-7 bg-[#FAFAF8] border-b border-[#E8E4DF] shrink-0">
          ...
          <div>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-[#F0F4F9] border border-[#13213c]/30 flex items-center justify-center text-[#13213c] shadow-2xs">
                {isEditMode ? <Sparkles className="w-5 h-5 text-[#13213c]" /> : <Plus className="w-5 h-5 text-[#13213c]" />}
              </div>
              <h2 className="text-xl sm:text-2xl font-black text-[#1C1917] tracking-tight">
                {isEditMode ? `تعديل المنتج: ${formData.name || product?.name}` : 'إضافة منتج جديد للمتجر'}
              </h2>
            </div>
            <p className="text-xs sm:text-sm text-[#78716C] mt-1.5 font-medium">
              {isEditMode
                ...
                : 'أدخل تفاصيل الهدية الجديدة، الصور، والأسعار لتظهر فوراً في المتجر للزبائن'}
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 self-start sm:self-auto">
            {onCancel && isInline && (
              <Button
                ...
                variant="outline"
                onClick={onCancel}
                className="h-10 px-4 rounded-xl border-[#E8E4DF] text-[#78716C] hover:text-[#1C1917] font-bold text-xs cursor-pointer"
              >
      ...
      {/* Body Content */}
      <div className={cn("p-5 sm:p-6 md:p-7 space-y-6 flex-1 overflow-y-auto text-start", !isInline && "max-h-[60vh]")}>
            ...
            <div className="lg:col-span-7 bg-white border border-[#E8E4DF] rounded-3xl p-5 sm:p-7 space-y-5 shadow-2xs">
              ...
              <div>
                ...
                <Textarea
                  ...
                  placeholder="اكتب وصفاً مفصلاً يبرز فخامة الهدية، تفاصيل التغليف، المواد المستخدمة، والمناسبات الملائمة لها..."
        ...
        {/* TAB 3: BADGES & VISIBILITY SETTINGS */}
        {activeTab === 'settings' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-7 space-y-4">
              ...
              <div className="flex items-center justify-between p-5 bg-[#FAFAF8] rounded-2xl border border-[#E8E4DF]">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#78716C] mt-0.5">يضيف شارة مميزة ويبرز المنتج في الصفحة الرئيسية</p>
                  </div>
                </div>
                <Switch
              ...
              <div className="flex items-center justify-between p-5 bg-[#FAFAF8] rounded-2xl border border-[#E8E4DF]">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl text-[#13213c] bg-[#F0F4F9] flex items-center justify-center">
                    <Sparkles className="w-5 h-5" />
                  </div>
                  <div>
                    <p className="text-sm font-black text-[#1C1917]">تمييز كـ &quot;منتج جديد&quot;</p>
                    <p className="text-xs text-[#78716C] mt-0.5">يضيف شارة براقة تلفت انتباه الزوار لتشكيلات الموسم</p>
                  </div>
                </div>
                <Switch
      ...
      {/* Footer Actions */}
      <div className="p-5 sm:p-6 bg-[#FAFAF8] border-t border-[#E8E4DF] flex items-center justify-between gap-3 shrink-0">
        {onCancel ? (
          <Button
            type="button"
            variant="outline"
        ...
        >
          {isSubmitting ? (
            <>
              ...
              {isEditMode ? 'جاري حفظ التعديلات...' : 'جاري إضافة المنتج...'}
            </>
          ) : (
            <>
              <Check className="w-4 h-4 ms-1.5" />
              {isEditMode ? 'حفظ التعديلات' : 'إضافة المنتج للمتجر'}
            </>
          )}
        </Button>
      </div>
