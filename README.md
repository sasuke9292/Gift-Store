...
Gift-Store/
├── prisma/
│   ├── schema.prisma          # تعريف جداول قاعدة البيانات والعلاقات
│   └── seed.ts                # بيانات تجريبية أولية (منتجات، تصنيفات، إعدادات)
├── public/                    # الوسائط والصور الثابتة والخطوط
├── docs/                      # التوثيق المعماري والتشغيلي التفصيلي
│   ├── ARCHITECTURE.md        # المعمارية البرمجية ومخطط العلاقات (ERD)
│   ├── ADMIN_GUIDE.md         # دليل إدارة المتجر والعمليات اليومية
│   ├── SETUP_AND_DEPLOYMENT.md# دليل التثبيت المحلي والنشر السحابي
│   └── API_AND_ACTIONS.md     # مرجع الـ Server Actions ونقاط الـ API
├── src/
│   ├── app/                   # مسارات التطبيق بنظام Next.js App Router
│   │   ├── (store)/           # واجهة المتجر الموجهة للمستخدمين
│   │   │   ├── cart/          # صفحة السلة والطلب
│   │   │   ├── checkout/      # صفحة الدفع والشحن
│   │   │   ├── category/      # صفحات الأقسام والكاتالوج
│   │   │   ├── product/       # صفحة تفاصيل المنتج وتنسيق الإهداء
│   │   │   ├── gift-finder/   # أداة مستكشف الهدايا الذكي
│   │   │   ├── track-order/   # تتبع مسار الشحنة برقم الطلب
│   │   │   └── home-client.tsx# الواجهة التفاعلية الفاخرة للصفحة الرئيسية
│   │   ├── admin/             # لوحة تحكم الإدارة الكاملة
│   │   │   ├── products/      # إدارة المنتجات مع البوكس العائم
│   │   │   ├── categories/    # إدارة التصنيفات والأقسام
│   │   │   ├── orders/        # إدارة الطلبات وإشعارات الواتساب
│   │   │   ├── hero-slides/   # استوديو شرائح السلايدر
│   │   │   ├── users/         # إدارة مدراء ومستخدمي النظام
│   │   │   └── settings/      # محرك إعدادات المتجر المنظم المبوب
│   │   ├── actions/           # إجراءات السيرفر (Server Actions)
│   │   │   ├── admin/         # عمليات السيرفر الخاصة بالإدارة
│   │   │   └── orders.ts      # معالجة الطلبات والتحقق
│   │   └── api/               # مسارات الـ API (Auth، رفع الملفات)
│   ├── components/            # مكتبة المكونات المشتركة
│   │   ├── admin/             # بطاقات ومخططات لوحة التحكم
│   │   ├── layout/            # الترويسة، الفوتر، القوائم الجانبية
│   │   ├── store/             # بطاقة المنتج، فلاتر الكاتالوج
│   │   └── ui/                # مكونات shadcn الأساسية (أزرار، حقول، نوافذ)
│   └── lib/                   # الدوال المساعدة والخدمات
│       ├── prisma.ts          # عميل Prisma Client الموحد
│       ├── store.ts           # مخزن Zustand للسلة والمفضلة
│       ├── whatsapp.ts        # منسق رسائل وأرقام الواتساب
│       └── utils.ts           # دوال دمج الكلاسات وتنسيق الأرقام
